...
//function triggerd when rows or columns are chan
	  ...
	  content+="<div id='row_id_"+i+"'><div class='row'><div class='large-5 columns'><input required id='rating_subq_"+i+"' type='text'></div><div class='large-6 columns'><span class='rating'>";
			  for(j=1;j<=$scope.cols;j++){
				content+="<input type='radio' class='rating-input'><label class='rating-star'></label>";  
				  }
				  content+="</span></div><div class='large-1 columns'><a href='#' onclick=delete_row('row_id_"+i+"'); ><span class='error'>Delete</span></a></div></div></div>";  
				}
 ...
 $scope.add_row=function(){
	 var existing_cont=document.getElementById("matrix").innerHTML;
	 	  existing_cont+="<div id='row_id_"+($scope.rows+1)+"'><div class='row'><div class='large-5 columns'><input required ng-model='rating_subq_"+($scope.rows+1)+"' id='rating_subq_"+($scope.rows+1)+"' type='text'></div><div class='large-6 columns'><span class='rating'>";
			  for(j=1;j<=$scope.cols;j++){
				existing_cont+="    <input type='radio' class='rating-input'><label class='rating-star'></label>";  
				  }
				  existing_cont+="</span></div><div class='large-1 columns'><a href='#' onclick=delete_row('row_id_"+($scope.rows+1)+"');><span  class='error'>Delete</span></a></div></div></div>"; 
				  $scope.rows++;
